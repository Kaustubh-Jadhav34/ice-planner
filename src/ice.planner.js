...
export class IcePlanner extends LitElement {
  ...
  constructor() {
    super();
    this.teamName='Penn State Youth'; this.logo='';
  ...
  // ---------- Storage & URL ----------
  _loadFromStorage() {
    try { const raw=localStorage.getItem(this._storageKey); if(raw){ Object.assign(this, JSON.parse(raw)); this._recalc(); } } catch(e){}
  }
  _saveToStorage() {
    try {
  ...
  _loadFromURL() {
    ...
    const getN=(k,d)=>{ const n=Number(sp.get(k)); return Number.isFinite(n)?n:d; };
    const getS=(k,d)=>{ const v=sp.get(k); return v!==null?v:d; };
    if (sp.toString().length>0) {
      this.teamName=getS('team',this.teamName); this.logo=getS('logo',this.logo);
      this.iceCost=getN('ice',this.iceCost); this.hours=getN('hrs',this.hours);
      this.feePct=getN('pct',this.feePct); this.feeFixed=getN('fix',this.feeFixed);
      this.coachCost=getN('coach',this.coachCost); this.jerseyCost=getN('jersey',this.jerseyCost);
      this.players=Math.max(1, getN('players',this.players)); this._recalc();
    }
  ...
  static get styles() {
    return css`
      :host{ display:block; --pad:16px; --card-bg:#fff; --card-fg:#0b1c5b; --muted:#475569;
             --accent:var(--ddd-theme-default-wonderPurple,#5632e6); --ring:#cbd5e1; }
      @media (prefers-color-scheme: dark){ :host{ --card-bg:#0b1220; --card-fg:#dbeafe; --muted:#93a3b8; --ring:#263449; } }
      .planner{ background:var(--card-bg); color:var(--card-fg); border:1px solid var(--ring); border-radius:16px;
                padding:calc(var(--pad)*1.5); display:grid; gap:16px; }
      header{ display:flex; gap:12px; align-items:center; flex-wrap:wrap; }
      header h2{ margin:0; font:800 1.4rem/1.2 system-ui,Arial,sans-serif; color:var(--accent); }
      header input[type="text"]{ flex:1; min-width:240px; max-width:420px; padding:8px 12px; border-radius:10px;
                                 border:1px solid var(--ring); color:var(--card-fg); background:transparent; }
      .grid{ display:grid; gap:16px; grid-template-columns:repeat(2,minmax(220px,1fr)); }
      @media (max-width:720px){ .grid{ grid-template-columns:1fr; } }
      .totals{ display:grid; gap:8px; border-top:1px dashed var(--ring); padding-top:12px; }
      .row{ display:flex; justify-content:space-between; }
      .label{ color:var(--muted); } .value{ font-weight:700; }
      ...
      img.logo{ height:40px; width:40px; object-fit:cover; border-radius:50%; border:1px solid var(--ring); }
    `;
  }
    ...
    return html`
      <div class="planner">
        <header>
          ...
          <h2>Ice Planner</h2>
          <input type="text" placeholder="Team name" .value=${this.teamName}
                 @input=${e=>this.teamName=e.target.value} />
          <input type="text" placeholder="Logo URL (optional)" .value=${this.logo}
        ...
        <div class="totals">
          ...
          <div class="share"><button class="btn" @click=${this._copyURL}>Copy sharable URL</button></div>
        </div>
      </div>
    `;
  }
